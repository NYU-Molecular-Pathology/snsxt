...
def run_task_list(analysis, task_list, extra_handlers = None):
    ...
    if background_jobs:
        monitor_background_jobs(background_jobs = background_jobs)

def monitor_background_jobs(background_jobs):
    '''
    ...
    '''
    logger.debug('Waiting for background qsub jobs to complete:\n{0}'.format([(job.id, job.name) for job in background_jobs]))

    completed_jobs, err_jobs = qsub.monitor_jobs(jobs = background_jobs)
    ...
    logger.debug('Validating completion status of background jobs...')
    job_validations = [job.validate_completion() for job in completed_jobs]
    if all(job_validations):
        logger.debug('All background jobs appear to have completed successfully')
    else:
